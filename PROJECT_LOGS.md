...
- Investigated the next local/cloud smoke failure: the latest log had fresh `Long press ended - processing...` entries without new processing errors, while recent temporary wav/debug-capture evidence showed the shared recording stop path was the common boundary before local/cloud diverged. Root cause: darwin `AudioRecorder.stop()` killed `rec` before registering the `close` listener, so a fast-exiting recorder could leave the promise unresolved and prevent both local Whisper and cloud ASR from starting.
- Added regression coverage in `tests/platform-index.test.js` for immediate `rec` exit after `SIGTERM`, then patched `src/main/platform/adapters/darwin/audio-recorder.js` to register `close` before signaling, settle once, and clear the force-kill timer. Verification after patch: the red test reproduced the hang, focused platform tests passed, `npm run verify` passed with 155/155 tests and coverage gate, `npm run build` passed, and packaged app inspection confirmed the recorder fix is present in `dist/mac-arm64/Kory Whisper.app`.
- Corrected the cloud-only diagnosis after user confirmed local Whisper works and only cloud mode shows the tray error. Compared the Aliyun provider adapter with the official Paraformer WebSocket Node.js example, which sends audio chunks on a 100 ms cadence before `finish-task`; the current implementation blasted all chunks synchronously and finished immediately, so settings connection tests could pass while real audio failed.
- Added `tests/aliyun-paraformer-engine.test.js` coverage for paced audio chunk streaming and provider-sized default chunks, then patched `src/main/asr/aliyun-paraformer-engine.js` with 4 KB chunks plus a configurable 100 ms inter-chunk wait. Verification after patch: red tests reproduced the missing pacing and oversized default chunk behavior, focused Aliyun tests passed, `npm run verify` passed with 157/157 tests and coverage gate, `npm run build` passed, and packaged app inspection confirmed the cloud pacing fix is present in `dist/mac-arm64/Kory Whisper.app`.
- Merged remote `origin/master` Windows runtime/release work into the local Aliyun BYOK cloud ASR line as one unified merge scope.
- Conflict decision: preserved the `ws` runtime dependency for DashScope WebSocket ASR while keeping remote Windows scripts (`smoke:windows`, `build:win:dir`, `build:win:release`).
- Conflict decision: combined `src/renderer/settings.html` ASR API-key preservation with the remote platform UI contract rendering path so Windows no longer inherits macOS-only shortcut/cue assumptions.
- Conflict decision: kept both Aliyun ASR defaults/tests and Windows smoke/runtime/profile tests, then added coverage proving win32 first-run defaults still use cloud ASR (`aliyun-paraformer`, `paraformer-realtime-v2`).
- Verification evidence recorded under `artifacts/merge-windows-asr-unification/verification-2026-04-19.md`: focused ASR/Windows/settings/composition slice passed with 67 tests, `npm run verify` passed with 220/220 tests and coverage gate, and `npm run build` passed with the expected unsigned macOS build warning.
