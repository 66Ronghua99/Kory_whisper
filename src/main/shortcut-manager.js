...
class ShortcutManager extends EventEmitter {
  constructor(options = {}) {
    ...
    this.longPressDuration = options.duration || 500;
  ...
  init() {
    ...
    try {
      ...
      this.keyboard.addListener((event, down) => {
        // 调试：记录所有按键事件（用于诊断）
        const isTargetKey = event.name === this.eventKey;
        if (isTargetKey || event.name.includes('COMMAND') || event.name.includes('META')) {
          console.log('[Shortcut] Raw event:', event.name, 'state:', event.state, 'down:', JSON.stringify(down));
        }
  ...
  destroy() {
    ...
    // 正确清理监听器
    if (this.keyboard) {
      // node-global-key-listener 没有明确的 removeListener 方法
      // 但可以通过设置为 null 来让 GC 回收
      this.keyboard = null;
    }
    this.isKeyDown = false;
    this.isRecording = false;
