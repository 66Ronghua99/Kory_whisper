...
test('canonical config manager merges platform profile defaults without changing the persisted config shape', () => {
  ...
  assert.equal(win32Defaults.asr.mode, 'cloud');
  assert.equal(win32Defaults.asr.cloud.provider, 'aliyun-paraformer');
  assert.equal(win32Defaults.asr.cloud.model, 'paraformer-realtime-v2');
...
test('profile defaults fall back to process.platform when no platform hint is provided', () => {
  ...
  const expectedByPlatform = {
    ...
    win32: {
      shortcutKey: 'RIGHT CONTROL',
      inputMethod: 'clipboard'
    }
  };
  const expected = expectedByPlatform[process.platform] || expectedByPlatform.darwin;

  assert.equal(defaults.shortcut.key, expected.shortcutKey);
  assert.equal(defaults.input.method, expected.inputMethod);
});
